    ...
    "design_docs": [
        {
            ...
            "sections": [
                ...
                },
                {
                    "heading": "Data Model",
                    "body": (
                        "The schema is designed around the pipeline — each stage has its own table rather than "
                        "a status column on a single fat record. "
                        "PACKETS is the top-level intake entity. Each packet contains one or more DOCUMENTS, "
                        "and each document tracks its current pipeline step explicitly. "
                        "EVENTS is the central audit spine — every table (documents, extractions, classifications, "
                        "matches, postings) holds a foreign key to EVENTS. Any document's full lifecycle — "
                        "from intake to final posting — is reconstructable from the event log alone. "
                        "EXTRACTED_DATA stores the AI output for each document: all fields pulled by the LLM, "
                        "plus two boolean flags (reached_poster, reached_final_queue) that track pipeline "
                        "progress without polling. "
                        "CLASSIFICATIONS records what the decision graph decided and, critically, "
                        "the path_followed through the graph — every classification carries a built-in "
                        "audit trail of which rules fired. "
                        "MATCHES is the validation bridge: extracted data is matched to records in the "
                        "external system before anything is posted. The AI never writes a posting "
                        "without a confirmed match. "
                        "POSTINGS is the header record for a completed posting; POSTING_BODY holds "
                        "the line-item detail (one row per service). "
                        "SETTLEMENTS and RECOMMENDED_ALLOWANCES are separate entities — "
                        "different payment types get different tables, not a type flag on a shared table."
                    ),
                },
                {
                    "heading": "What's Hard",
